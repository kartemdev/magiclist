...
(async () => {
  ...
  const gitPullResonse = await prompts({
    type: 'toggle',
    name: 'value',
  ...
  if (remoteVersion !== localVersion && gitPullResonse.value) {
    resetResponse = await prompts({
      type: 'toggle',
      name: 'value',
      message: `Текущая локальная версия отличаеться от версии в origin/master, cбросить до ${remoteVersion}?\n`,
      initial: false,
  ...
  if (resetResponse.value && gitPullResonse.value) {
